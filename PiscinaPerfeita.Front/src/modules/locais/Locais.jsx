...
function LocalForm({ initial, onSubmit, onCancel, loading }) {
  const [form, setForm] = useState(
    initial ?? {
      nome: "", descricao: "", telefone: "", observacoes: "",
    ...
    <form onSubmit={handleSubmit}>
      <FormGrid>
        ...
        <FormField label="País">
          <input type="text" style={inputStyle} value={form.pais} onChange={set("pais")} />
        </FormField>
        <FormField label="Observações" fullWidth>
          <input type="text" style={inputStyle} value={form.observacoes} onChange={set("observacoes")} />
        </FormField>
      </FormGrid>
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 16 }}>
        <Button variant="ghost" onClick={onCancel} type="button">Cancelar</Button>
        <Button variant="primary" type="submit" disabled={loading}>
          {loading ? "Salvando…" : initial ? "Salvar alterações" : "Cadastrar local"}
        </Button>
      </div>
    </form>
  );
...
export default function Locais() {
  ...
  async function handleDelete(id) {
    if (!confirm("Excluir este local? Usuários vinculados perderão o acesso a ele.")) return;
    try {
  ...
  const columns = [
    ...
    {
      key: "_cidadeEstado", label: "Cidade/UF",
  ...
  return (
    <div>
      ...
      <Toolbar>
        <SearchInput value={search} onChange={setSearch} placeholder="Buscar local…" />
      </Toolbar>

      <Card noPadding>
        <DataTable columns={columns} data={filtered} emptyMessage="Nenhum local cadastrado." />
      </Card>
