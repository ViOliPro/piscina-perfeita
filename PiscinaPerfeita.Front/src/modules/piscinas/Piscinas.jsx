...
function PiscinaForm({ usuarios, initial, onSubmit, onCancel, loading }) {
  const [form, setForm] = useState(
    initial ?? { nome: "", volumeLitros: "", profundidadeMedia: "", usuarioId: "" }
  );
  ...
  function handleSubmit(e) {
    ...
    onSubmit({
      ...
      profundidadeMedia: form.profundidadeMedia  ? parseFloat(form.profundidadeMedia) : null,
    });
  }
    ...
    <form onSubmit={handleSubmit}>
      <FormGrid>
        <FormField label="Nome *" fullWidth>
          <input required type="text" placeholder="Ex.: Piscina Olímpica Principal"
            style={inputStyle} value={form.nome} onChange={set("nome")} />
        </FormField>
        <FormField label="Volume (litros)">
          <input type="number" min="0" placeholder="Ex.: 1000000"
            style={inputStyle} value={form.volumeLitros} onChange={set("volumeLitros")} />
        </FormField>
        <FormField label="Profundidade média (m)">
          <input type="number" step="0.1" min="0" placeholder="Ex.: 2.0"
            style={inputStyle} value={form.profundidadeMedia} onChange={set("profundidadeMedia")} />
        </FormField>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 16 }}>
        ...
        <Button variant="primary" type="submit" disabled={loading}>
          {loading ? "Salvando…" : initial ? "Salvar alterações" : "Cadastrar piscina"}
        </Button>
      </div>
    </form>
  );
...
export default function Piscinas() {
  ...
  useEffect(() => {
    async function load() {
      try {
        setLoading(true);
        const [p, u] = await Promise.all([piscinaService.listar(), usuarioService.listar()]);
        setPiscinas(p ?? []);
        setUsuarios(u ?? []);
  ...
  async function handleSave(dto) {
    try {
      setSaving(true);
      if (modal.editing) {
        const atualizada = await piscinaService.atualizar(modal.editing.id, dto);
        setPiscinas((prev) => prev.map((p) => p.id === atualizada.id ? atualizada : p));
      } else {
  ...
  async function handleDelete(id) {
    if (!confirm("Excluir esta piscina? Todos os dados relacionados serão afetados.")) return;
    try {
  ...
  const columns = [
    { key: "nome", label: "Nome" },
    {
      key: "volumeLitros", label: "Volume (L)",
      render: (v) => v ? v.toLocaleString("pt-BR") : "—",
    },
    {
      key: "profundidadeMedia", label: "Profundidade média",
      render: (v) => v ? `${v} m` : "—",
    },
    {
    ...
    { key: "usuario", label: "Responsável", render: (_, r) => r.usuario?.nome ?? "—" },
    {
      ...
      render: (_, r) => (
        <div style={{ display: "flex", gap: 6 }}>
          <Button variant="ghost" size="sm" onClick={() => setModal({ open: true, editing: r })}>
            Editar
          </Button>
  ...
  return (
    <div>
      ...
      <Toolbar>
        <SearchInput value={search} onChange={setSearch} placeholder="Buscar piscina…" />
      </Toolbar>

      <Card noPadding>
        <DataTable columns={columns} data={filtered} emptyMessage="Nenhuma piscina cadastrada." />
      </Card>
