...
function UsuarioForm({ initial, onSubmit, onCancel, loading }) {
  ...
  const logadoEhSuperAdmin =
    (usuarioLogado?.role ?? usuarioLogado?.Role) === ROLES.ADMIN;
  ...
  const [form, setForm] = useState(
    initial
      ...
      : {
          ...
          senha: "",
          cpf: "",
          perfil: String(PERFIS.VISUALIZADOR),
          localId: "",
        },
  );
    ...
    <form onSubmit={handleSubmit}>
      <FormGrid>
        ...
        <FormField label="Papel *">
          <select
            ...
            disabled={
              isEdit &&
              !logadoEhSuperAdmin &&
              parseInt(form.role) === ROLES.ADMIN
            }
            style={inputStyle}
            value={form.role}
            onChange={set("role")}
          >
            <option value={String(ROLES.USER)}>
            ...
            {(logadoEhSuperAdmin ||
              ...
              <option value={String(ROLES.ADMIN)}>
        ...
        {!isEdit && parseInt(form.role) === ROLES.ADMIN && (
          <FormField label="ID do Local (opcional)" fullWidth>
            <input
              type="text"
              placeholder="Deixe em branco para não vincular a um local específico"
              style={inputStyle}
              value={form.localId}
              onChange={set("localId")}
            />
          </FormField>
        )}
      </FormGrid>
      ...
      >
        <Button
          ...
          onClick={onCancel}
          type="button"
          permission={PERMISSIONS.USUARIOS.CREATE}
        >
          Cancelar
        </Button>
        <Button
...
// ----------------------------------------------------------
function VinculoPendente({ vinculo, locaisDisponiveis, onVincular, saving }) {
  ...
  return (
    ...
    >
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 13, fontWeight: 600 }}>Vínculo pendente</div>
        <div style={{ fontSize: 12, color: "#5a6b7a" }}>
          {PERFIL_LABELS[vinculo.perfil] ?? "—"} — ainda sem local definido
        </div>
      </div>
      <select
      ...
      <Button
        variant="primary"
        size="sm"
        disabled={!localId || saving}
        onClick={() => onVincular(vinculo, localId)}
...
function VinculosLocaisModal({ usuario, open, onClose }) {
  ...
  // Oficializa um vínculo pendente (criado automaticamente sem LocalId,
  // seja na criação do usuário por um SuperAdmin) — define o Local.
  async function handleVincularPendente(vinculo, localId) {
    try {
      setSaving(true);
      setError(null);
      const atualizado = await usuarioLocalService.atualizar(vinculo.id, {
  ...
  const localNome = (localId) =>
    locaisDisponiveis.find((l) => l.id === localId)?.nome ??
    "Local desconhecido";

  const vinculosPendentes = vinculos.filter((v) => !v.localId);
  const vinculosAtivos = vinculos.filter((v) => v.localId);

  return (
    <Modal
      open={open}
      onClose={onClose}
      title={`Locais de ${usuario?.nome ?? ""}`}
    >
          ...
          >
            {vinculosPendentes.map((v) => (
              <VinculoPendente
                ...
                onVincular={handleVincularPendente}
                saving={saving}
              />
            ))}
            ...
            <FormGrid>
              <FormField label="Vincular a um novo local">
                <select
                  ...
                  value={novo.localId}
                  onChange={(e) =>
                    setNovo((f) => ({ ...f, localId: e.target.value }))
                  }
                >
                  <option value="">Selecione…</option>
                  {locaisDisponiveis
                    .filter(
                      (l) => !vinculosAtivos.some((v) => v.localId === l.id),
                    )
                    .map((l) => (
                      <option key={l.id} value={l.id}>
                        {l.nome}
              ...
              <FormField label="Perfil">
                <select
                  ...
                  value={novo.perfil}
                  onChange={(e) =>
                    setNovo((f) => ({ ...f, perfil: e.target.value }))
                  }
                >
                  {Object.entries(PERFIL_LABELS).map(([valor, label]) => (
                    <option key={valor} value={valor}>
...
export default function Usuarios() {
  ...
  const logadoEhSuperAdmin =
    (usuarioLogado?.role ?? usuarioLogado?.Role) === ROLES.ADMIN;
  const logadoEhAdministrador =
    (usuarioLogado?.perfil ?? usuarioLogado?.Perfil) === PERFIS.ADMINISTRADOR;
  const [usuarios, setUsuarios] = useState([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState(null);
  const [modal, setModal] = useState({ open: false, editing: null });
  const [vinculosModal, setVinculosModal] = useState({
    open: false,
    usuario: null,
  });
  ...
  const buttonDisabled = logadoEhSuperAdmin
    ? false
    : logadoEhAdministrador
      ? false
      : true;
      ...
      render: (_, r) => (
        <div style={{ display: "flex", gap: 6 }}>
          <Button
            variant="ghost"
            size="sm"
            onClick={() => setVinculosModal({ open: true, usuario: r })}
            permission={PERMISSIONS.USUARIOS.VINCULO}
            disabled={buttonDisabled}
            title={
              !logadoEhSuperAdmin || !logadoEhAdministrador
                ? "Somente SuperAdmin pode gerenciar vínculos de local"
                : undefined
            }
          >
            Locais
          ...
          <Button
            ...
            permission={PERMISSIONS.USUARIOS.EDIT}
          >
            Editar
          </Button>
          <Button
