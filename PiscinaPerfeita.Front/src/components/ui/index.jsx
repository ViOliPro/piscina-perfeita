    ...
    >
      <div
        style={{
          ...
          animation: isMobile
            ? "pp-sheet-up .25s ease-out"
            : "pp-modal-in .2s ease-out",
        }}
      >
        <style>{`
        ...
        <div style={{ padding: isMobile ? "16px 16px 8px" : "18px 20px" }}>
          {children}
        </div>
...
function MobileCard({ columns, row, index }) {
  ...
  const [titleCol, ...detailCols] = columns.filter(
    (c) => c.key !== "_acoes" && c.key !== "_edit" && c.key !== "_status",
  );
  const actionCol = columns.find(
    (c) => c.key === "_acoes" || c.key === "_edit",
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        {statusCol && <div>{statusCol.render(row[statusCol.key], row)}</div>}
      </div>
          ...
          {data.map((row, i) => (
            ...
            >
              {columns.map((col) => (
                <td
                  key={col.key}
                  style={{ padding: "10px 12px", color: "var(--text-primary)" }}
                >
                  {col.render
                    ? col.render(row[col.key], row)
                    : (row[col.key] ?? "—")}
  ...
  return (
    ...
    >
      <div>
        ...
        {description && (
          <p style={{ fontSize: 13, color: "#6B8CAE", marginTop: 2 }}>
            {description}
          </p>
        )}
      </div>
      {action && (
        <div style={{ display: "flex" }}>
          ...
          {isMobile ? <div style={{ width: "100%" }}>{action}</div> : action}
        </div>
      )}
    </div>
  );
